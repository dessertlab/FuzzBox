...
uint32_t qemu_plugin_get_cpu_register(unsigned int cpu_index,
    ...
    uint32_t reg_value = 0;
        ...
        reg_value = (uint32_t) qemu_get_cpu(cpu_index)->env_ptr->active_tc.gpr[reg];
    #elif defined(TARGET_ARM)
        reg_value = (uint32_t) qemu_get_cpu(cpu_index)->env_ptr->regs[reg];
    #endif
...
uint64_t qemu_plugin_get_cpu_register_64(unsigned int cpu_index,
    ...
    #if defined(TARGET_MIPS64) || defined(TARGET_MIPS64EL)
        reg_value = (uint64_t) qemu_get_cpu(cpu_index)->env_ptr->active_tc.gpr[reg];
    #elif defined(TARGET_AARCH64)
        reg_value = (uint64_t) qemu_get_cpu(cpu_index)->env_ptr->xregs[reg];
    #endif
...
void qemu_plugin_set_cpu_register(unsigned int cpu_index,
    ...
    uint32_t *register_address = NULL;

    #if defined(TARGET_MIPS) || defined(TARGET_MIPSEL)
        register_address = (uint32_t*) qemu_get_cpu(cpu_index)->env_ptr->active_tc.gpr + reg;
    #elif defined(TARGET_ARM)
        register_address = (uint32_t*) qemu_get_cpu(cpu_index)->env_ptr->regs + reg;
    #endif
...
void qemu_plugin_set_cpu_register_64(unsigned int cpu_index,
                                     unsigned int reg,
                                     uint64_t value) {
    //void* cpu = qemu_get_cpu(cpu_index);
    // Trova l'indirizzo del registro all'interno della memoria della CPU
    //uint32_t* register_address = (uint32_t*)((cpu + offset1 + offset2 + reg * 4));
    uint64_t *register_address = NULL;

    #if defined(TARGET_MIPS64) || defined(TARGET_MIPS64EL)
        register_address = (uint64_t*) qemu_get_cpu(cpu_index)->env_ptr->active_tc.gpr + reg;
    #elif defined(TARGET_AARCH64)
        register_address = (uint64_t*) qemu_get_cpu(cpu_index)->env_ptr->xregs + reg;
    #endif
