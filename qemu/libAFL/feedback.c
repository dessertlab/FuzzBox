...
float __attribute__((hot)) afl_feedback_cov_is_interesting(afl_feedback_t *feedback, afl_executor_t *fsrv)
{
  ...
  (void)fsrv;
  /*if (!FEEDBACK_MODE)
    return 0; // può esser implementato in maniera più modulare "staccando" il modulo feedback dal main ma mi dava problemi quindi nessun input è interessante e amen
  */
