...
void afl_mutfunc_delete_bytes(afl_mutator_t *mutator, afl_input_t *input)
{
  ...
  /*Capped number of bytes to delete as a fix to cap on clone bytes*/
  if (del_len>2){
    del_len=2;
  }
...
void afl_mutfunc_clone_bytes(afl_mutator_t *mutator, afl_input_t *input)
{
  ...
  if (actually_clone)
  {
    ...
    clone_len = choose_block_len(rand, size);
     if(clone_len>95){//realloc bug workaround
       clone_len=95;
     }
  ...
  else
  {
    ...
    if(clone_len>40){//realloc bug workaround // se input len seed troppo piccolo questo fa crashare! vorrei fare debug :(
       clone_len=40;
     }
    printf("\nafl_mutfunc_clone_bytes  --- ELSE ---  CLONE LEN: %d\n", clone_len);
    
     if(clone_len<3){//realloc bug workaround // se input len seed troppo piccolo questo fa crashare! vorrei fare debug :(
       clone_len=3;
     }
...
afl_ret_t afl_mutator_scheduled_add_havoc_funcs(afl_mutator_scheduled_t *mutator)
{
  ...
  // update: in realtà sembra dipendere adlla lunghezza del seed
  // se il seed è troppo corto, realloc
